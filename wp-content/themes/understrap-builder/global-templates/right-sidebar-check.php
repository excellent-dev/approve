...
<?php if ($us_b_show_sidebar_right > 0) : ?>
  ...
  <div class="col-md-<?php echo esc_attr($us_b_show_sidebar_right); ?> <?php echo esc_attr($us_b_sidebars_widget_margin_class); ?> widget-area" id="right-sidebar" role="complementary">
    ...
    <div>
      ...
      <?php 
      // получаем все термины из таксономии cities
      ...
      if ( !empty( $terms ) && !is_wp_error( $terms ) ) {
        ...
        foreach ($terms as $term) {
          $i++;
          $term_list .= '<a href="' . get_term_link( $term ) . '" title="' . sprintf(__('Показать всю недвижимость в городе %s', 'my_localization_domain'), $term->name) . '">' . $term->name . '</a>';
          if ($count != $i) {
            $term_list .= ' <br> ';
          }
          else {
